...
export default function Lotes() {
  ...
  async function loadAnimaisLote(loteId: number) {
    if (loteExpandido === loteId) {
      setLoteExpandido(null)
      return
    }
    try {
      const data = await fetchAnimais({ lote_id: String(loteId) })
      setAnimaisLote(data)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNovoLote({ nome: '', descricao: '', modalidade: 'CORTE' })
      setShowForm(false)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido')
    }
                    ...
                    <tbody>
                      {animaisLote.map((animal) => (
                        <tr key={animal.id}>
                          ...
                          <td className={animal.gmd && animal.gmd >= 1 ? 'text-green-400' : 'text-amber'}>
