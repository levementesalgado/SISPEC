...
import { TrendingUp, AlertTriangle, CheckCircle, Scale, Activity, Thermometer, Droplets, Clock, Target } from 'lucide-react'
...
export default function DashboardOperacional() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await fetchDashboardOperacional()
        setKpis(data.kpis)
      ...
      } catch { /* fallback silencioso pra dados mock */ }
      setLoading(false)
    }
    load()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-serif text-paper">Dashboard Operacional</h1>
          <p className="text-white/40 text-sm mt-1">12 KPIs em tempo real · atualização a cada 5s</p>
        </div>
